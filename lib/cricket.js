...
function to3(n) { return String(Math.round(n * 1000) / 1000); }
...
// --- Super Over (tie-breaker) support ---
// innings layout: [0]=team1 normal, [1]=team2 normal, then pairs of Super Overs.
// For each pair: even index (2,4,6..) = team2 bats first, odd index (3,5,7..) = team1 bats.
// The team that batted second in the match bats first in the Super Over (standard rule).
function superOverQuota() { return 1; } // Super Over = 1 over = 6 legal balls
function isSuperOverBallIndex(i) { return i >= 2; }

// Full match decision including Super Over tie-breakers.
// Returns { winner, loser, marginType, text, tie, superOver }.
function matchOutcomeFull(m, season, data) {
  const inns = m.innings || [];
  const quota = season ? quotaOvers(season) : OVERS_PER_QUOTA_FALLBACK;
  const t1 = m.team1, t2 = m.team2;
  const inn1 = inningsSummary(inns[0], quota, squadSizeFor(t1, data));
  const inn2 = inningsSummary(inns[1], quota, squadSizeFor(t2, data));
  const base = matchOutcome(inn1, inn2, t1, t2, data);
  if (!base) return null;
  if (base.marginType !== 'tie') {
    return { ...base, tie: false, superOver: false };
  }
  // Tied after normal play. Walk through Super Over pairs.
  let soCount = 0;
  for (let i = 2; i + 1 < inns.length; i += 2) {
    const aBalls = inns[i], bBalls = inns[i + 1];
    const aTeam = i % 2 === 0 ? t2 : t1;
    const bTeam = i % 2 === 0 ? t1 : t2;
    if (!Array.isArray(bBalls) || bBalls.length === 0) break;
    const aInn = inningsSummary(aBalls, superOverQuota(), squadSizeFor(aTeam, data));
    const bInn = inningsSummary(bBalls, superOverQuota(), squadSizeFor(bTeam, data));
    const sob = matchOutcome(aInn, bInn, aTeam, bTeam, data);
    soCount++;
    if (sob && sob.marginType !== 'tie') {
      const label = soCount === 1 ? 'Super Over' : 'Super Over ' + soCount;
      const marginTxt = sob.marginType === 'runs'
        ? ' by ' + sob.margin + ' run' + (sob.margin === 1 ? '' : 's')
        : ' by ' + sob.margin + ' wicket' + (sob.margin === 1 ? '' : 's');
      return { winner: sob.winner, loser: sob.loser, margin: sob.margin, marginType: sob.marginType, text: sob.winner + ' won the ' + label + marginTxt + ' — match tied', tie: true, superOver: true, superOverNumber: soCount };
    }
  }
  // All recorded Super Overs also tied (unresolved yet).
  return { winner: null, loser: null, margin: 0, marginType: 'tie', text: 'Match tied', tie: true, superOver: soCount > 0, superOverNumber: soCount };
}

function superOversComplete(m, data) {
  if (!Array.isArray(m.innings)) return 0;
  const len = m.innings.length;
  if (len <= 2) return 0;
...
function standingsForSeason(season, data) {
  ...
  const rows = teams.map(name => ({ team: name, played: 0, won: 0, lost: 0, tied: 0, nrr: '0', points: 0, position: 0 }));
  const completed = (season.matches || []).filter(m => m.status === 'completed');
  completed.forEach(m => {
    const inn1 = inningsSummary(m.innings && m.innings[0], quotaOvers(season), squadSizeFor(m.team1, data));
    const inn2 = inningsSummary(m.innings && m.innings[1], quotaOvers(season), squadSizeFor(m.team2, data));
    const out = matchOutcomeFull(m, season, data);
    const r1 = rows.find(r => r.team === m.team1);
    const r2 = rows.find(r => r.team === m.team2);
    if (r1) { r1.played++; if (out && out.marginType === 'tie' && !out.winner) { r1.tied++; r1.points += 1; } else if (out && out.winner === m.team1) { r1.won++; r1.points += 2; } else if (out && out.winner) { r1.lost++; } }
    if (r2) { r2.played++; if (out && out.marginType === 'tie' && !out.winner) { r2.tied++; r2.points += 1; } else if (out && out.winner === m.team2) { r2.won++; r2.points += 2; } else if (out && out.winner) { r2.lost++; } }
  });
  // NRR
...
function allPlayerAggregates(data, seasonOnly) {
  ...
  // Fielding credits (catches / run outs / stumpings) from raw ball list
  (data.seasons || []).forEach(season => {
    if (seasonOnly && season.id !== seasonOnly.id) return;
    [...((season.matches || [])), ...((season.playoff || []))].forEach(m => {
      if (m.status === 'upcoming') return;
      (m.innings || []).forEach(inArr => {
        (inArr || []).forEach(b => {
          if (!b.isWicket || !b.fielder) return;
          const wt = (b.wicketType || '').toLowerCase().replace(/\s+/g, ' ');
          if (['caught', 'stumped', 'run out', 'runout', 'lbw stumping'].indexOf(wt) !== -1) {
            const a = field[b.fielder] = field[b.fielder] || { catches: 0 };
            a.catches++;
          }
        });
      });
...
function seasonPerformance(season, data) {
  ...
  [].concat(season.matches || [], season.playoff || []).forEach(m => {
    ...
    m.motm = mp.motm;
    if (mp.motm) motms.push(Object.assign({}, mp.motm, { match: m.match || m.id, date: m.date || '' }));
    Object.keys(mp.byPlayer).forEach(name => {
      const a = mp.byPlayer[name];
      const t = totals[name] = totals[name] || { points: 0, runs: 0, wickets: 0, catches: 0, fours: 0, sixes: 0, motmAwards: 0, matches: 0, team: '' };
      t.points += a.points;
      t.runs += a.runs;
      t.wickets += a.wickets;
      t.catches += a.catches;
  ...
  Object.keys(totals).forEach(name => {
    ...
    if (!potm || t.points > potm.points || (t.points === potm.points && t.runs > potm.runs)) {
      potm = { player: name, points: t.points, runs: t.runs, wickets: t.wickets, catches: t.catches, fours: t.fours, sixes: t.sixes, motmAwards: t.motmAwards, matches: t.matches, team: t.team };
    }
  });
  return { motms, potm };
}
...
function seasonChampion(season, data) {
  ...
  if (fin && fin.status === 'completed' && fin.innings && fin.innings.length >= 2) {
    const out = matchOutcomeFull(fin, season, data);
    if (out && out.winner) {
