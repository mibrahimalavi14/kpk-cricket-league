...
// to a COPY of the data object. Never mutates the input (which may be shared/cached).
...
function buildSeasonStats(agg, players) {
  ...
  const wktsList = Object.keys(agg.bowls).map(name => {
    ...
    return {
      ...
      avg: nat(b.avg),
      eco: nat(b.eco),
      sr: b.wickets > 0 ? nat(b.balls / b.wickets) : '0',
      best: b.best,
      matches: b.matches,
      innings: b.inns,
      balls: b.balls,
...
function attach(data) {
  ...
  (data.seasons || []).forEach(season => {
    ...
    allMatches.forEach(m => {
      ...
      m.superOvers = c.superOversComplete(m, data);
      m.superOverLegs = [];
      if (m.superOvers > 0 && Array.isArray(m.innings)) {
        for (let i = 2; i < m.innings.length; i++) {
          const legTeam = i % 2 === 0 ? m.team2 : m.team1;
          m.superOverLegs.push({
            leg: Math.floor((i - 2) / 2) + 1,
            team: legTeam,
            inn: c.inningsSummary(m.innings[i], c.superOverQuota(), c.squadSizeFor(legTeam, data)),
          });
        }
      }
      if (m.status === 'completed' && m.innings.length >= 2) {
        const out = c.matchOutcomeFull(m, season, data);
        if (out) {
          m.outcome = out;
    ...
    // Player of the Season is revealed only after every league + playoff match is complete
    const allCompMatches = [].concat(season.matches || [], season.playoff || []);
    const seasonComplete = allCompMatches.length > 0 && allCompMatches.every(m => m.status === 'completed');
    season.playerOfSeason = seasonComplete ? perf.potm : null;
    ...
    if (season.playoff && season.playoff.length >= 2 && sorted.length >= 3) {
      ...
      if (leagueDone) {
        ...
        if (q1.status === 'completed' && q1.team1 !== 'TBD' && q1.team2 !== 'TBD') {
          const qOut = c.matchOutcomeFull(q1, season, data);
  ...
  (data.players || []).forEach(p => {
    p.age = c.ageFromDob(p.dob);
    const b = career.bats[p.name] || {};
    const bw = career.bowls[p.name] || {};
    const f = career.field[p.name] || {};
    p.matches = Math.max(b.matches || 0, bw.matches || 0);
    p.battingInnings = b.inns || 0;
    p.innings = b.inns || 0;
    p.runs = b.runs || 0;
    p.ballsFaced = b.balls || 0;
    p.average = f2(b.avg);
    p.strikeRate = f2(b.sr);
    p.dismissals = b.dismissals || 0;
    ...
    p.bowlingOvers = bw.overs || '0';
    p.ballsBowled = bw.balls || 0;
    p.runsGiven = bw.runs || 0;
    p.bowlingAverage = f2(bw.avg);
    p.economyRate = f2(bw.eco);
    p.bowlingStrikeRate = bw.wickets > 0 ? nat(bw.balls / bw.wickets) : '0';
...
module.exports = { attach, buildSeasonStats, ageFromDob };