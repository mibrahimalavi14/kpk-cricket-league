...
// Admin - Separate Live Scoring page (lists every match, scoring from there only)
router.get('/admin/scoring', authMiddleware, async (req, res) => {
  const data = await loadAdminData();
  const all = [];
  (data.seasons || []).forEach(season => {
    (season.matches || []).forEach(m => all.push({ match: m, seasonId: season.id, seasonName: season.name + ' - ' + season.year, isPlayoff: false }));
    (season.playoff || []).forEach(m => all.push({ match: m, seasonId: season.id, seasonName: season.name + ' - ' + season.year, isPlayoff: true }));
  });
  const active = all.find(x => x.match.status === 'live') || all.find(x => x.match.status !== 'completed');
  res.render('admin/scoring', { data, all, active });
});
...
router.post('/admin/season/:id/match/:mid/start', authMiddleware, async (req, res) => {
  ...
  if (season) {
    const found = locateMatch(season, req.params.mid);
    if (found && found.match.toss === undefined) {
      const m = found.match;
      m.status = 'live';
      if (!Array.isArray(m.innings) || m.innings.length === 0) m.innings = [[]];
      const winner = req.body.toss || m.team1;
      const choice = req.body.tossChoice === 'bowl' ? 'bowl' : 'bat';
      m.toss = winner;
      m.tossChoice = choice;
      const batFirst = choice === 'bat' ? winner : (winner === m.team1 ? m.team2 : m.team1);
      if (m.team1 !== batFirst) {
        const tmp = m.team1; m.team1 = m.team2; m.team2 = tmp;
      }
      m.batFirst = m.team1;
      m.result = '';
      await saveData(data);
...
router.post('/admin/season/:id/match/:mid/ball', authMiddleware, async (req, res) => {
  ...
  const isSO = c.isSuperOverBallIndex(lastIdx);
  const quota = isSO ? c.superOverQuota() : c.quotaOvers(season);
  const battTeam = isSO
    ? (lastIdx % 2 === 0 ? match.team2 : match.team1)
    : (lastIdx === 0 ? match.team1 : match.team2);
  ...
  if (season) {
    ...
    if (found) {
      ...
      if (match.innings && match.innings.length >= 2) {
        const full = c.matchOutcomeFull(match, season, data);
        if (full && full.tie && !full.winner) {
          // Tied and no deciding Super Over scored yet -> open the next Super Over leg (one innings at a time).
          match.innings.push([]);
          match.status = 'live';
          match.inSuperOver = true;
          await saveData(data);
        } else if (full) {
          match.outcome = full;
          match.result = full.text;
          match.status = 'completed';
          match.inSuperOver = false;
          await saveData(data);
        }
      }
    }
...
router.post('/admin/season/:id/match/:mid/reset', authMiddleware, async (req, res) => {
  ...
  if (season) {
    ...
    if (found) {
      ...
      found.match.inSuperOver = undefined;
      found.match.inn1 = undefined;
      found.match.inn2 = undefined;
      found.match.outcome = undefined;
      await saveData(data);
    }
  }
  res.redirect('/admin/season/' + req.params.id + '/match/' + req.params.mid);
